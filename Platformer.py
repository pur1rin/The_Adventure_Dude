    ...
    for événement in pygame.event.get(): #boucle d'événement
        ...
        if événement.type == KEYDOWN: #si le bouton est appuyé alors les actions attribuées à ce bouton s'enclencheront
            if événement.key == K_RIGHT  or événement.key == K_d :
                a_droite = True
            if événement.key == K_LEFT or événement.key == K_a :
                a_gauche = True
            if événement.key == événement.key == K_UP  or événement.key == K_w :
                if tair < 6:
            ...
            if collisions['left'] == True: #saut contre le mur de de gauche
                if événement.key == K_w or événement.key == K_UP :
                    if vitesse_y > 2:
                        vitesse_y = -3.65
            ...
            if événement.key == K_c:
                code = int(input('Enter the secret code : (If you answer wrongly, nothing will happpen just do not press cancel or close this tab!!)')) #codes pour accéder aux différents niveaux avec leurs points de spawn respectifs
                if code == 6848:
